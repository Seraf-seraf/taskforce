document.addEventListener('DOMContentLoaded', () => {
    ...
    const starRating = document.querySelector('.active-stars');
    ...
    if (starRating) {
        ...
        starRating.addEventListener('click', function (event) {
            ...
            if (star.matches('span')) {
                stars.forEach((element, i) => {
                    if (i < star.dataset.star) {
                        stars[i].className = 'fill-star';
                    } else {
                        stars[i].className = '';
                    }
                });
            ...
            const input = starRating.previousSibling;
            input.value = star.getAttribute('data-star');
        });
    }
})